...
Tale deformazione può essere definita \textbf{elastica} se le forze applicate s
...
$$
Nel configurazione reale, alla molla è appeso un piattello (di massa $m_p$).
...
v = \frac{x}{l}V
$$
Si può quindi scrivere:Possiamo quindi
$$
...
Si consideri per semplicità l'equazione omogenea (abbiamo visto la soluzione della non omogenea):
$$
...
\end{figure}
%%%%fine metodo statico dioproco%%%%

\subsubsection{Metodo dinamico}
\subsubsection*{Calcolo senza usare un modello}

Calcoliamo i valori dei periodi medi $\bar{T_i}$ per ogni $m_i$ associato con il rispettivo errore $\sigma_{\bar{T_i}}$.
Calcoliamo anche i valori $\bar{T_{0}}$ corrispondenti ai periodi medi a piattino scarico, ogni set di masse ha il suo in modo che siano indipendenti.
...
Utilizzando gli stessi valori per i periodi del modello precedente abbiamo calcolato la costante 
...
$$\sigma_k^2 = k^2\left[\left(\frac{\sigma_{M_tot}}{M_{tot}}\right)^2+4\left(\frac{\sigma_{T_i}}{T_i}\right)^2\right]$$
E $\sigma_{M_{tot}}:$
